...
Количественно учитываются **только запчасти** (расходники, ставятся в авто). Оборудование —
многоразовая оснастка цеха (сканеры, калибровочные стенды), поэтому по количеству **не
учитывается и не списывается**: фиксируется лишь факт наличия (есть / нет).

Остатки запчастей ведутся в таблице `stock_items` по ключу `тип + sku` (sku = артикул, а при
его отсутствии — наименование). При старте склад засеивается запчастями из справочника
`RetrofitCatalog` (`stock.initial-quantity`, по умолчанию 10 шт. на позицию; засев идемпотентный).

- **Проверка наличия запчастей** сравнивает потребность заявки с остатком: позиция «в наличии»,
  если на складе хватает количества. Оборудование считается доступным по умолчанию.
- **Выдача запчастей** списывает количество со склада, **поступление по заказу** — пополняет.
  Оборудование при выдаче не списывается.
- Просмотр и корректировка: `GET /api/v1/stock`, `POST /api/v1/stock` (установить остаток;
  удобно обнулить позицию запчасти, чтобы продемонстрировать ветку заказа).
